...
#Alternate method using nested queries (Reward Users who are also active) 
select au.customer_id, au.email, au.first_name
from
(Select C.*,A.phone
from customer C Join address A ON C.address_id=A.address_id
where C.active=1
group by 1) as au JOIN 
(select R.customer_id, count(R.rental_id), max(rental_date)
from rental R 
group by 1
having count(R.rental_id)>= 30) as ru 
ON 
...
#Alternate method using nested queries (All Rewards Users with Phone)
select ru.customer_id, c.email, au.phone
from 
(select R.customer_id, count(R.rental_id), max(rental_date)
from rental R 
...
having count(R.rental_id)>= 30) as ru 
JOIN 
customer c ON ru.customer_id=c.customer_id
LEFT JOIN
(Select C.*,A.phone
from customer C Join address A ON C.address_id=A.address_id
where C.active=1
